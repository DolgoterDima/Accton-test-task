...
<script setup>
const NO_USERS_HEAD_TEXT = "no head";
...
const getUsersHead = (user) => {
  const userHeadId = user.head;
  const usersHead = users.value.find((item) => item.id === userHeadId);
